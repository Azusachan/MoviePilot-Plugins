...
} = useRuntimeData(api, notify, "CloudSubscribeFork", {
...
async function playHistory(itemId) {
  ...
  try {
    const result = await api.get(`plugin/CloudSubscribeFork/history/play/${encodeURIComponent(itemId)}`)
...
async function retryHistory() {
  ...
  try {
    const result = await api.post("plugin/CloudSubscribeFork/history/retry", {
      time: record.time,
