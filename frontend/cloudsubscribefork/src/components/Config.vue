          ...
          <div class="sidebar-brand">
            <div class="sidebar-brand-main">
              ...
              <div class="sidebar-brand-text">
                ...
                <span class="sidebar-brand-subtitle">CloudSubscribeFork</span>
              </div>
            </div>
...
async function fetchInstalledPluginDetail() {
  const currentPluginId = props.pluginId || props.sourcePluginId || "CloudSubscribeFork";
  let detail = null;
  try {
...
async function testSearchProxy() {
  ...
  try {
    const response = unwrapResponse(
      await api.post("plugin/CloudSubscribeFork/search/proxy/test", {
        proxy,
...
async function testAutoSubscribeProxy() {
  ...
  try {
    const response = unwrapResponse(
      await api.post("plugin/CloudSubscribeFork/auto_subscribe/proxy/test", {
        proxy,
...
async function requestResourceUrl(item) {
  const response = unwrapResponse(
    await api.post("plugin/CloudSubscribeFork/search/unlock", {
...
async function loadPreviewDirectory(parentId, breadcrumbs, requestId = ++previewRequestId) {
  ...
  try {
    const response = unwrapResponse(
      await api.post("plugin/CloudSubscribeFork/search/preview", {
...
async function loadOptions(scope = "base", {force = false, refresh = false} = {}) {
  ...
  const request = (async () => {
    ...
    const response = unwrapResponse(await api.get(`plugin/CloudSubscribeFork/ui_options?${query}`))
    if (response.success === false) {
...
async function refreshAccount(accountKey, { silent = false } = {}) {
  ...
  try {
    const response = unwrapResponse(
      await api.post("plugin/CloudSubscribeFork/account/refresh", {
        key: normalizedKey,
      }),
    )
    if (response.success === false) {
...
async function save() {
  ...
  try {
    ...
    const response = unwrapResponse(await api.post("plugin/CloudSubscribeFork/config/save", payload))
    if (response.success === false) {
...
async function startHdhiveOAuth() {
  ...
  try {
    const response = unwrapResponse(await api.post("plugin/CloudSubscribeFork/hdhive/oauth/start", hdhiveOAuthPayload()))
    if (response.success === false) {
...
async function exchangeHdhiveOAuth(callbackData = {}) {
  ...
  try {
    const response = unwrapResponse(
      await api.post("plugin/CloudSubscribeFork/hdhive/oauth/exchange", {
        ...hdhiveOAuthPayload(),
...
async function searchTmdbCandidates() {
  ...
  testError.value = ""
  try {
    const response = unwrapResponse(await api.post("plugin/CloudSubscribeFork/search/tmdb", { title }))
    if (response.success === false) {
...
async function testSource(candidate) {
  ...
  try {
    const response = unwrapResponse(
      await api.post("plugin/CloudSubscribeFork/search/test", {
...
async function testAutoSubscribe(provider) {
  ...
  try {
    const response = unwrapResponse(
      await api.post("plugin/CloudSubscribeFork/auto_subscribe/test", {
