import {enabled} from "./helpers.js";

export function createTransferSection(options) {
  return {
    value: "transfer",
    title: "转存设置",
    icon: "mdi-cloud-download-outline",
    groups: [
      {
        title: "订阅范围",
        icon: "mdi-filter-outline",
        fields: [
          {
            key: "subscribe_filter_mode",
            label: "订阅筛选模式",
            type: "select",
            items: [
              { title: "排除所选订阅", value: "exclude" },
              { title: "仅处理所选订阅", value: "include" },
            ],
            cols: 4,
          },
          {
            key: "exclude_subscribes",
            label: "排除订阅",
            type: "select",
            items: options.subscribes,
            multiple: true,
            searchable: true,
            cols: 8,
            show: (config) => config.subscribe_filter_mode === "exclude",
          },
          {
            key: "include_subscribes",
            label: "指定订阅",
            type: "select",
            items: options.subscribes,
            multiple: true,
            searchable: true,
            cols: 8,
            show: (config) => config.subscribe_filter_mode === "include",
          },
        ],
      },
      {
        title: "转存控制",
        icon: "mdi-tune-variant",
        fields: [
          {
            key: "cross_transfer_enabled",
            label: "跨盘资源自动转存",
            type: "switch",
            hint: "资源链接与目标网盘不一致时，自动尝试秒传；未命中则进行普通中继上传。任务统一显示在订阅任务中。",
            cols: 4,
          },
          {
            key: "organize_after_transfer",
            label: "转存后自动整理",
            type: "switch",
            hint: "关闭后，转存或离线下载的文件保留在中转目录，不执行重命名、移动、STRM 生成与媒体刮削。",
            cols: 4,
          },
          {
            key: "skip_other_season_dirs",
            label: "跳过其他季目录",
            type: "switch",
            hint: "与目标季数不一致的文件自动跳过。",
            cols: 4,
          },
          {
            key: "organize_subtitles",
            label: "整理伴随字幕",
            type: "switch",
            hint: "关闭后，转存或离线下载时不匹配或整理伴随字幕，直接跳过字幕处理流程。",
            cols: 4,
          },
          {
            key: "subtitle_traditional_to_simplified",
            label: "字幕自动繁转简",
            type: "switch",
            hint: "下载并整理伴随字幕时，将字幕内容自动由繁体转换为简体中文保存。",
            cols: 4,
          },
          {
            key: "anime_pack_preferred",
            label: "动漫优先下载合集",
            type: "switch",
            hint: "当动漫番剧存在完结合集（如 [01-12 合集]、全集）时，优先选择合集资源进行离线下载与转存。",
            cols: 4,
          },
          {
            key: "offline_timeout",
            label: "离线下载超时（分钟）",
            hint: "提交给网盘离线下载后超过该时间未完成则标记超时退出，默认 30 分钟。",
            type: "number",
            min: 10,
            max: 1440,
            cols: 4,
          },
          {
            key: "cross_transfer_info",
            type: "info",
            label: "跨盘秒传说明",
            lines: [
              "仅当资源链接所属网盘与当前目标网盘不一致时触发；同盘资源继续使用原生分享转存。",
              "115 使用 SHA1；123、光鸭、天翼使用 MD5。只有源文件校验和与目标算法一致时才尝试秒传。",
              "秒传未命中或源端缺少可用校验和时，自动下载到本地缓存并普通上传；成功后清理，失败或取消时保留以便恢复。",
              "任务统一显示在订阅任务中，并区分校验、秒传、源盘下载、目标盘上传、完成、失败和取消状态。",
            ],
            cols: 12,
            show: enabled("cross_transfer_enabled"),
          },
          {
            key: "transfer_task_batch_size",
            label: "任务内每批处理文件数",
            hint: "仅控制任务内拆批大小；当前任务会继续处理后续批次直至完成。",
            type: "number",
            min: 1,
            max: 1000,
            cols: 4,
          },
          {
            key: "subscription_concurrency",
            label: "订阅并发数",
            type: "number",
            min: 1,
            max: 5,
            cols: 4,
          },
          {
            key: "transfer_risk_cooldown",
            label: "转存风控冷却（秒）",
            type: "number",
            min: 60,
            max: 86400,
            cols: 4,
          },
          {
            key: "batch_size",
            label: "分享转存每批数量",
            type: "number",
            min: 1,
            cols: 4,
          },
          {
            key: "batch_interval",
            label: "转存批次间隔（秒）",
            type: "number",
            min: 0,
            max: 60,
            cols: 4,
          },
          {
            key: "cross_transfer_download_threads",
            label: "下载线程数",
            type: "number",
            min: 1,
            max: 10,
            cols: 4,
            show: enabled("cross_transfer_enabled"),
          },
          {
            key: "cross_transfer_max_concurrent",
            label: "同时跨盘任务数",
            type: "number",
            min: 1,
            max: 10,
            cols: 4,
            show: enabled("cross_transfer_enabled"),
          },
          {
            key: "cross_transfer_media_types",
            label: "允许跨盘转存类型",
            type: "select",
            items: [
              { title: "电影", value: "movie" },
              { title: "电视剧", value: "tv" },
            ],
            cols: 4,
            multiple: true,
            show: enabled("cross_transfer_enabled"),
          },
        ],
      },
      {
        title: "本地路径与 STRM",
        icon: "mdi-folder-play-outline",
        hint: "配置本地媒体根路径及转存后的 STRM 生成方式。",
        fields: [
          {
            key: "local_resource_path",
            label: "本地媒体根路径",
            placeholder: "/strm",
            cols: 6,
          },
          {
            key: "cross_transfer_download_path",
            label: "跨盘中继缓存目录",
            placeholder: "留空使用系统临时目录",
            cols: 6,
          },
          {
            key: "strm_generate_enabled",
            label: "转存后直接生成 STRM",
            type: "switch",
            cols: 4,
          },
          {
            key: "nfo_scrape_enabled",
            type: "switch",
            label: "刮削生成 NFO",
            cols: 4,
          },
          {
            key: "image_scrape_enabled",
            type: "switch",
            label: "刮削生成图片",
            cols: 4,
          },
          {
            key: "strm_base_url",
            label: "STRM 基础地址",
            placeholder: "http://172.17.0.1:9527",
            cols: 8,
            show: enabled("strm_generate_enabled"),
          },
          {
            key: "strm_url_template",
            label: "STRM URL 模板",
            type: "textarea",
            hint: "115变量：{pickcode}；夸克变量：{file_id}；光鸭变量：{file_id}、{gcid}；123变量：{file_id}、{md5}、{size}、{s3_key_flag}；通用变量：{base_url}、{file_name}、{file_path}。插件仅按模板生成 STRM，不代理播放请求。",
            cols: 12,
            show: enabled("strm_generate_enabled"),
          },
        ],
      },
    ],
  }
}
