...
class SyncRuntimeService(OwnerDelegator):
        ...
        if task_snapshot:
            try:
                ...
                Thread(
                    ...
                    daemon=True,
                    name="cloudsubscribefork-safe-postprocess-stop",
                ).start()
            except Exception as error:
                with self._sync_tasks_lock:
                    current = self._sync_tasks.get(task_id)
    ...
    def _monitor_offline_task_groups(
        ...
        with ThreadPoolExecutor(
                ...
                thread_name_prefix="cloudsubscribefork-offline",
        ) as executor:
            futures = {
                executor.submit(
            ...
            if pending_count > 0:
                if scheduler and scheduler.running:
                    scheduler.modify_job(
                        "CloudSubscribeFork_OfflineMonitor",
                ...
                scheduler.add_job(
                    ...
                    id="CloudSubscribeFork_OfflineMonitor",
                    next_run_time=datetime.datetime.now(
                        tz=pytz.timezone(settings.TZ)
