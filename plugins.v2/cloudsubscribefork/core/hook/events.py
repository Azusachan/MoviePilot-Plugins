...
class PluginEventHandler(OwnerDelegator):
    ...
    def on_transfer_complete(self, event: Event):
        ...
        Thread(
            ...
            daemon=True,
            name="cloudsubscribefork-pt-upgrade",
        ).start()
    ...
    def on_message_action(self, event: Event) -> None:
        ...
        Thread(
            ...
            daemon=True,
            name="cloudsubscribefork-telegram-link-select",
        ).start()
    ...
    def on_plugin_action(self, event: Event):
        ...
        if not action.startswith("cloudsubscribefork_"):
            return

        if action == "cloudsubscribefork_status":
        ...
        if action == "cloudsubscribefork_sync":
        ...
        if action == "cloudsubscribefork_auto_subscribe":
            Thread(
                target=self._run_remote_auto_subscribe,
                args=(dict(event_data),),
                daemon=True,
                name="cloudsubscribefork-command-auto-subscribe",
            ).start()
            return

        if action == "cloudsubscribefork_links":
            ...
            Thread(
                ...
                daemon=True,
                name="cloudsubscribefork-command-links",
            ).start()
            self._post_command_message(
                event_data,
                "【网盘订阅】正在校验资源",
        ...
        if action == "cloudsubscribefork_link_select":
            ...
            Thread(
                ...
                daemon=True,
                name="cloudsubscribefork-command-link-select",
            ).start()
            self._post_command_message(
                event_data,
                "【网盘订阅】正在提交资源",
        ...
        if action == "cloudsubscribefork_checkin":
            ...
            Thread(
                ...
                daemon=True,
                name="cloudsubscribefork-command-checkin",
            ).start()
            self._post_command_message(
                event_data,
                "【网盘订阅】正在签到",
        ...
        if action == "cloudsubscribefork_checkin_history":
            args = str(event_data.get("arg_str") or "").strip().lower().split()
            if len(args) > 2:
                self._post_command_message(
                    event_data,
        ...
        if action == "cloudsubscribefork_cache_clear":
    ...
    def on_resource_download(self, event: Event):
        ...
        if all_plugin_managed and policy == "allow":
            if self._has_pending_cloud_target(
                ...
                event_data.source = "CloudSubscribeFork-重复资源拦截"
        ...
        if all_plugin_managed and policy == "cloud":
            ...
            event_data.cancel = True
            event_data.source = "CloudSubscribeFork-平台资源下载接管"
            event_data.reason = (
                "平台资源已提交插件离线下载"
        ...
        if all_plugin_managed and policy == "block":
            ...
            event_data.cancel = True
            event_data.source = "CloudSubscribeFork-平台资源下载拦截"
            event_data.reason = (
