...
from .schemas import (
    CloudSubscribeForkCacheClearInput,
    CloudSubscribeForkCheckinInput,
    CloudSubscribeForkCheckinHistoryInput,
    CloudSubscribeForkConfigUpdateInput,
    CloudSubscribeForkLinksInput,
    CloudSubscribeForkPerformanceInput,
    CloudSubscribeForkResourceSearchInput,
    CloudSubscribeForkResourceSelectInput,
    CloudSubscribeForkStatusInput,
    CloudSubscribeForkSyncInput,
)


def _plugin():
    plugin = PluginManager().running_plugins.get("CloudSubscribeFork")
    return plugin if plugin and getattr(plugin, "_agent_enabled", True) else None


class CloudSubscribeForkStatusTool(MoviePilotTool):
    name: str = "cloudsubscribefork_status"
    ...
    args_schema: Type[BaseModel] = CloudSubscribeForkStatusInput
...
class CloudSubscribeForkSyncTool(MoviePilotTool):
    name: str = "cloudsubscribefork_start_sync"
    ...
    args_schema: Type[BaseModel] = CloudSubscribeForkSyncInput
...
class CloudSubscribeForkLinksTool(MoviePilotTool):
    name: str = "cloudsubscribefork_process_links"
    tags: list[str] = [
        ToolTag.Write,
        ToolTag.Resource,
    ...
    description: str = (
        ...
        "仅处理用户明确提供的链接；"
        "搜索工具返回的候选必须改用 cloudsubscribefork_select_resources，禁止复制或改写候选链接。"
    )
    args_schema: Type[BaseModel] = CloudSubscribeForkLinksInput
...
class CloudSubscribeForkCheckinTool(MoviePilotTool):
    name: str = "cloudsubscribefork_checkin"
    tags: list[str] = [ToolTag.Write, ToolTag.Plugin]
    description: str = (
        "立即执行网盘订阅助手签到。可指定渠道，省略时签到全部已启用渠道。"
        "normal 为普通签到；gambler/lucky 为渠道配置的其他签到模式。"
    )
    args_schema: Type[BaseModel] = CloudSubscribeForkCheckinInput
...
class CloudSubscribeForkCheckinHistoryTool(MoviePilotTool):
    name: str = "cloudsubscribefork_checkin_history"
    tags: list[str] = [ToolTag.Read, ToolTag.Plugin]
    description: str = (
        "按渠道列举网盘订阅助手的签到详情，包括执行时间、状态、模式、积分变化、"
        "当前积分和累计签到天数。不会返回 HTTP、错误码或验证码等内部信息。"
    )
    args_schema: Type[BaseModel] = CloudSubscribeForkCheckinHistoryInput
...
class CloudSubscribeForkResourceSearchTool(MoviePilotTool):
    name: str = "cloudsubscribefork_search_resources"
    ...
    description: str = (
        ...
        "收到选择后使用本工具返回的 search_id 调用 cloudsubscribefork_select_resources。"
        "未关联现有订阅的搜索只能展示和推荐，不能直接转存。"
    )
    args_schema: Type[BaseModel] = CloudSubscribeForkResourceSearchInput
    ...
    async def run(
        ...
        self._agent_context["reply_mode"] = "cloudsubscribefork_search"
        return json.dumps(result, ensure_ascii=False, indent=2, default=str)


class CloudSubscribeForkResourceSelectTool(MoviePilotTool):
    name: str = "cloudsubscribefork_select_resources"
    tags: list[str] = [
        ToolTag.Write,
        ToolTag.Resource,
        ToolTag.Subscription,
    ...
    description: str = (
        "提交用户已确认的网盘候选资源。只能使用 cloudsubscribefork_search_resources 最近一次"
    ...
    args_schema: Type[BaseModel] = CloudSubscribeForkResourceSelectInput
...
class CloudSubscribeForkCacheClearTool(MoviePilotTool):
    name: str = "cloudsubscribefork_clear_cache"
    ...
    args_schema: Type[BaseModel] = CloudSubscribeForkCacheClearInput
...
class CloudSubscribeForkPerformanceTool(MoviePilotTool):
    name: str = "cloudsubscribefork_performance"
    ...
    args_schema: Type[BaseModel] = CloudSubscribeForkPerformanceInput
...
class CloudSubscribeForkConfigUpdateTool(MoviePilotTool):
    name: str = "cloudsubscribefork_update_config"
    ...
    require_admin: bool = True
    args_schema: Type[BaseModel] = CloudSubscribeForkConfigUpdateInput
