...
class CloudSubscribeForkStatusInput(BaseModel):
...
class CloudSubscribeForkSyncInput(BaseModel):
...
class CloudSubscribeForkLinksInput(BaseModel):
...
class CloudSubscribeForkCheckinInput(BaseModel):
...
class CloudSubscribeForkCheckinHistoryInput(BaseModel):
    provider: Optional[str] = Field(
        default=None,
...
class CloudSubscribeForkResourceSearchInput(BaseModel):
    subscribe_id: Optional[int] = Field(
        default=None,
        gt=0,
...
class CloudSubscribeForkResourceSelectInput(BaseModel):
    search_id: str = Field(
        ...,
        min_length=1,
        max_length=32,
        description="cloudsubscribefork_search_resources 返回的搜索 ID",
    )
...
class CloudSubscribeForkCacheClearInput(BaseModel):
    confirm: bool = Field(
        default=False,
...
class CloudSubscribeForkPerformanceInput(BaseModel):
...
class CloudSubscribeForkConfigUpdateInput(BaseModel):
