"""CloudSubscribeFork 插件业务数据与可恢复运行状态的 ORM 入口。"""
...
class CloudSubscribeForkDataStore:
    ...
    def __init__(self, owner):
        ...
        self.manager = CloudSubscribeForkDatabaseManager(
            owner.get_data_path() / "cloudsubscribefork.db"
        )
        self.repositories = CloudSubscribeForkRepositories(self.manager)
    ...
    def initialize(self) -> None:
        with self._lock:
            ...
            if repaired:
                logger.info(f"CloudSubscribeFork 历史媒体分组已修复：{repaired} 条")
