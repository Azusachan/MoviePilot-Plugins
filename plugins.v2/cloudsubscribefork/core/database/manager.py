"""CloudSubscribeFork 独立 SQLite 引擎与会话生命周期。"""
...
from .models import CloudSubscribeForkBase
...
class DbOper:
    ...
    def __init__(
            self,
            manager: "CloudSubscribeForkDatabaseManager",
            db: Session = None,
    ):
...
class CloudSubscribeForkDatabaseManager:
    ...
    def init_db(self) -> None:
        ...
        if self._engine is None:
            raise RuntimeError("CloudSubscribeFork 数据库引擎未初始化")
        CloudSubscribeForkBase.metadata.create_all(bind=self._engine)
    ...
    def update_db(self) -> None:
        ...
        config.attributes["target_metadata"] = CloudSubscribeForkBase.metadata
    ...
    def session(self, *, write: bool = False) -> Generator[Session, None, None]:
        ...
        if self._session_factory is None:
            raise RuntimeError("CloudSubscribeFork 数据库会话工厂未初始化")
        ...
        with self._lock:
            if self._engine is not None:
                try:
                    ...
                    logger.debug(f"CloudSubscribeFork WAL checkpoint 失败：{error}")
