...
from .core.storage import CloudSubscribeForkDataStore
...
class CloudSubscribeFork(_PluginBase):
    """网盘订阅助手插件。"""

    # 插件名称
    plugin_name = "网盘订阅助手 · Azusa"
    # 插件描述
    ...
    # 插件版本
    plugin_version = "1.5.7.47.3356382566.10"
    # 插件作者
    plugin_author = "odomu"
    # 作者主页
    author_url = "https://github.com/odomu"
    # 插件配置项ID前缀
    plugin_config_prefix = "cloudsubscribefork_"
    plugin_order = 21
    auth_level = 1
    ...
    def _get_data_store(self) -> CloudSubscribeForkDataStore:
        store = self.__dict__.get("_cloudsubscribefork_data_store")
        if store is None:
            store = CloudSubscribeForkDataStore(self)
            self.__dict__["_cloudsubscribefork_data_store"] = store
        return store
    ...
    def get_data(self, key: Optional[str] = None, plugin_id: Optional[str] = None) -> Any:
        ...
        if target_plugin == self.__class__.__name__ and key:
            if CloudSubscribeForkDataStore.handles(key):
    ...
    def _apply_plugin_config(
        ...
        if reset_runtime:
            ...
            self._sync_operation_executor = ThreadPoolExecutor(
                max_workers=1,
                thread_name_prefix="cloudsubscribefork-sync-operation",
            )
